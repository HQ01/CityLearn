    ...
    for building in buildings:
        ...
        if building.dhw_heating_device.nominal_power == 'autosize':
            ...
            if isinstance(building.dhw_heating_device, HeatPump):
                ...
                building.dhw_heating_device.cop_heating = building.dhw_heating_device.eta_tech * building.dhw_heating_device.t_target_heating / (
                            building.dhw_heating_device.t_target_heating - (building.sim_results['t_out'] + 273.15))
                ...
                building.dhw_heating_device.nominal_power = max(
                    building.sim_results['dhw_demand'] / building.dhw_heating_device.cop_heating)

            # If the device is an electric heater
            elif isinstance(building.dhw_heating_device, ElectricHeater):
                building.dhw_heating_device.nominal_power = max(
                    building.sim_results['dhw_demand'] / building.dhw_heating_device.efficiency)
        ...
        if building.cooling_device.nominal_power == 'autosize':
            building.cooling_device.cop_cooling = building.cooling_device.eta_tech * building.cooling_device.t_target_cooling / (
                        building.sim_results['t_out'] + 273.15 - building.cooling_device.t_target_cooling)
            ...
            building.cooling_device.nominal_power = max(
                building.sim_results['cooling_demand'] / building.cooling_device.cop_cooling)
    ...
    for uid, attributes in zip(data, data.values()):
        if uid in building_ids:
            ...
            building = Building(buildingId=uid, dhw_storage=dhw_tank, cooling_storage=chilled_water_tank,
                                dhw_heating_device=electric_heater, cooling_device=heat_pump)
            ...
            building.sim_results['solar_gen'] = attributes['Solar_Power_Installed(kW)'] * data[
            ...
            a_low, a_high = [], []
            for state_name, value in zip(buildings_states_actions[uid]['actions'],
...
class CityLearn(gym.Env):
    def __init__(self, building_attributes, solar_profile, building_ids, buildings_states_actions=None,
                 simulation_period=(0, TIME_PERIOD-1), cost_function=['quadratic']):
        ...
        self.buildings, self.observation_spaces, self.action_spaces = building_loader(building_attributes,
                                                                                      solar_profile, building_ids,
                                                                                      self.buildings_states_actions)
        self.action_track = {}
    ...
    def step(self, actions):
        ...
        self.electric_consumption_dhw_storage = np.append(self.electric_consumption_dhw_storage,
                                                          elec_consumption_dhw_storage)
        self.electric_consumption_cooling_storage = np.append(self.electric_consumption_cooling_storage,
        ...
        self.electric_consumption_cooling = np.append(self.electric_consumption_cooling,
                                                      elec_consumption_cooling_building)
        self.electric_consumption_appliances = np.append(self.electric_consumption_appliances,
                                                         elec_consumption_appliances)
    ...
    def cost(self):
        ...
        if self.cost_rbc is None:
            env_rbc = CityLearn(self.building_attributes, self.solar_profile, self.building_ids,
        ...
        if '1-load_factor' in self.cost_function:
            cost['1-load_factor'] = np.mean([1 - np.mean(self.net_electric_consumption[i:i + int(TIME_PERIOD / 12)]) /
                                             np.max(self.net_electric_consumption[i:i + int(TIME_PERIOD / 12)])
                                             for i in range(0, len(self.net_electric_consumption),
                                                            int(TIME_PERIOD / 12))]) / self.cost_rbc['1-load_factor']

        if 'peak_to_valley_ratio' in self.cost_function:
            cost['peak_to_valley_ratio'] = np.median(
                [self.net_electric_consumption[i:i + 24].max() / self.net_electric_consumption[i:i + 24].min() for i in
                 range(0, len(self.net_electric_consumption), 24)]) / self.cost_rbc['peak_to_valley_ratio']
